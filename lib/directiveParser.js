...
function parseBody(body) {
  ...
  for(var index in lines) {
    ...
    if (matches && matches[1]) {
      ...
      if('include'.indexOf(name) > -1) {
        directives.push([index, "include", args]);
      }
    }
  ...
  for(var i=0;i<directives.length;i++) {
    ...
    lastDirectiveIndex = index;
  }

  if(index < lines.length-1) {
    var c2 = lines.slice(index+1, lines.length).join("\n");
    expandedDirectives.push([parseInt(index) + 1, 'include_inline', {contents: c2}]);
  }
...
function directiveParser(filePath, data) {
  ...
  for(i=0;i<patterns.length;i++) {
    if(patterns[i].type == 'include_self' || patterns[i].type == 'include_inline') {
      found = true;
    }
  }
