...
var Lingon = function(rootPath) {
  ...
  this.registerTask('clean', function(callback) {
    rimraf(path.join(this.rootPath, this.buildPath), function(error) {
      if (!error) {
        log('[Info] The ' + this.buildPath + ' folder has been cleaned');
      } else {
        log(chalk.yellow('[Error] The ' + this.buildPath + ' folder could not be cleaned'));
      }
...
Lingon.prototype.build = function(callback, requestPaths) {
  ...
  var streamList = sourceFiles.map(function(sourceFile) {
...
Lingon.prototype.postProcess = function(sourceFile) {
  // console.log('process', sourceFile.name)
  var _this = this;
