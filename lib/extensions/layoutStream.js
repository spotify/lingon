...
module.exports = function layoutStream(env, context, layoutPath) {
  ...
  function getLayoutPromise(file, templatePath, absoluteLayoutPath, relativeLayoutPath, context) {
  ...
  function layoutStreamFn(file, cb) {
    ...
    getTemplatePromise(file, context).then(function(file) {
      templateFile = file;
      return getLayoutPromise(file, relativeTemplatePath, absoluteLayoutPath, relativeLayoutPath, context);

    }).then(function(layoutFile) {
      layoutData = layoutFile.contents.toString();

      var filename = path.basename(file.path);
      var newFilename = env.extensionRewriter.transform(filename, [env.preProcessors]);
