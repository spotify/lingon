...
ENV.prototype.getSourcePaths = function(whitelist) {
  ...
  // If we are requesting only one path, let's speed things up
  // by using the trail.find method instead of traversing the 
  // entire tree.
  if(whitelist.length == 1) {
    var singlePath = this.trail.find(whitelist[0]);
    if(!!singlePath) {
      paths.push(path.relative(this.rootPath, singlePath));
    }
