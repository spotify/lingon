...
var addProcessor = function(arrayModifierFn, extension, fileMatches, factory) {
  ...
  this.processors[extension][arrayModifierFn]({ fileMatches: fileMatches, pipe: factory });
  ...
  if(extension && this.processors[extension]) {
    if(!fileMatches) {
      ...
      for(var i=this.processors[extension].length; i--;) {
        var isFileValid = !fileMatches || (this.processors[extension][i].fileMatches && this.processors[extension][i].fileMatches.toString() == fileMatches.toString());
